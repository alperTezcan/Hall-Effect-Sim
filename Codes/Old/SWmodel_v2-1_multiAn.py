...
class SWmodel_multiAn:
    def __init__(self, ani_num, eventname):
    ...
    def setup_anisotropy(self):
        # Create multiple anisotropy system
        self.anisotropies = np.array([[0, 0, 0]])
        for i in range(self.ani_num):
            print(f"Info for {i+1}-th axis")
            ani_coef = float(input("Anisotropy field (coef x 0.5): "))
            theta_ani = np.radians(float(input("ThetaHan in degrees: ")))
            phi_ani = np.radians(float(input("PhiHan in degrees: ")))
            self.anisotropies = np.append(self.anisotropies, 
                                          [[0.5 * ani_coef, theta_ani, phi_ani]], 
                                          axis=0)
        print("model_created")
    ...
    def setup_output_directory(self):
        ...
        event_name = f"Axes-{self.ani_num}_{self.eventname}"
        self.output_dir = f"../Outputs/py/v2_1_multiAni/{event_name}/Plots"
    ...
    def funEner(self, Happ, ThetaH, PhiH, ThetaM, PhiM, Hd):
        ...
        for i in range(self.ani_num):
            result += self.funAni(self.anisotropies[i][0], 
                                  self.anisotropies[i][1], 
                                  self.anisotropies[i][2], ThetaM, PhiM)
        return result
...
if __name__ == "__main__":
    ...
    ani_num = int(input("How many isotropies are there: "))
    eventname = str(input("Experiment keyword: "))
    
    # Initialize model
    model = SWmodel_multiAn(ani_num, eventname)
